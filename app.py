...
data['Country'] = data['Combined_Key'].apply(lambda x: x.split(',')[-1].strip() if ',' in x else x)
...
# Group by 'Last_Update' and 'Country' to get total Active and Death cases per country per date
aggregated_data = data.groupby(['Last_Update', 'Country'], as_index=False).agg({'Active': 'sum', 'Deaths': 'sum'})

# Aggregate again by date to get the total global values per day
trend_data = aggregated_data.groupby('Last_Update', as_index=False).agg({'Active': 'sum', 'Deaths': 'sum'})
trend_data = trend_data.sort_values(by='Last_Update')
...
def time_series():
    ...
    ax.plot(trend_data['Last_Update'], trend_data['Active'], marker='o', linestyle='-', color='orange', label='Active Cases')
    ax.plot(trend_data['Last_Update'], trend_data['Deaths'], marker='s', linestyle='-', color='red', label='Death Cases')
...
def correlation_matrix():
    ...
    corr_matrix = data[['Confirmed', 'Deaths', 'Recovered', 'Active', 'Incident_Rate', 'Case_Fatality_Ratio']].corr()
