...
def score_resume(resume_data, job_description_data):
    ...
    resume_text_keywords = set(resume_data['full_text'].lower().split())
    job_keywords = set(job_description_data['keywords'])

    keyword_score = len(resume_text_keywords & job_keywords) / len(job_keywords) * 100 if job_keywords else 0
    ...
    total_score = 0.3 * keyword_score + 0.3 * skills_score + 0.2 * experience_score + 0.2 * education_score + 0.1 * structure_score
    return {
...
def generate_feedback(resume_data, job_description_data):
    ...
    missing_experience = job_keywords - resume_experience - missing_skills
    missing_education = job_keywords - resume_education - missing_skills - missing_experience
