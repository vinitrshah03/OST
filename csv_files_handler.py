...
def process_csv(file_name):
    ...
    try:
        ...
        if "Combined_Key" in df.columns:
            df["Country"] = df["Combined_Key"].apply(lambda x: x.split(",")[-1].strip() if isinstance(x, str) else x)
            df = df.dropna(subset=["Country"])
            invalid_countries = ["Winter Olympics 2022", "Summer Olympics 2020", "Diamond Princess", "MS Zaandam"]
        ...
        if "Last_Update" in df.columns and "Active" in df.columns and "Deaths" in df.columns:
            time_series_df = df.groupby("Last_Update", as_index=False).agg({"Active": "sum", "Deaths": "sum"})
            time_series_df = time_series_df.sort_values(by="Last_Update")

            fig, ax = plt.subplots(figsize=(10, 5))
            ax.plot(time_series_df["Last_Update"], time_series_df["Active"], marker="o", linestyle="-", color="orange", label="Active Cases")
            ax.plot(time_series_df["Last_Update"], time_series_df["Deaths"], marker="s", linestyle="-", color="red", label="Death Cases")
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(GRAPHS_DIR, f"time_series_{file_name}.png"))
            plt.close()

        # Generate bar chart if 'Incident_Rate' is present
        if "Country" in df.columns and "Incident_Rate" in df.columns:
            country_data = df.groupby("Country", as_index=False)["Incident_Rate"].mean()
            global_avg = country_data["Incident_Rate"].mean()

            fig, ax = plt.subplots(figsize=(18, 6))
            ax.bar(country_data["Country"], country_data["Incident_Rate"], color="skyblue", width=0.6)
            ax.axhline(global_avg, color="red", linestyle="--", linewidth=2, label=f"Global Avg: {global_avg:.2f}")
            exceeding_countries = country_data[country_data["Incident_Rate"] > global_avg]
            ax.set_xticks(exceeding_countries.index)
            ax.set_xticklabels(exceeding_countries["Country"], rotation=90, fontsize=9, ha="right", rotation_mode="anchor")
        ...
        correlation_columns = ["Confirmed", "Deaths", "Recovered", "Active", "Incident_Rate", "Case_Fatality_Ratio"]
        ...
        if len(available_columns) > 1:
            ...
            ax.set_title(f"Covid-19 Correlation Matrix - {file_name}", fontsize=14)
            plt.tight_layout()
            plt.savefig(os.path.join(GRAPHS_DIR, f"correlation_{file_name}.png"))
            plt.close()
